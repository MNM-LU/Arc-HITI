...
class align_local4():
    ...
    def align(self):
        alignment, score, start_end_positions = local_pairwise_align(DNA(self.ref),DNA(self.amplicon),gap_open_penalty=self.gop,gap_extend_penalty = self.gep)
        out_align = ('-'*start_end_positions[0][0])+str(alignment[1])+('-'*(len(self.ref)-start_end_positions[0][1]-1))
        return(out_align)
    ...
    def align_data(self):
        ...
        for seq_i in range(len(self.full_df.iloc[:,-1])):
            ...
            if self.rev_compl=="True":
                ...
                print(f'seq_obj_: {str(seq_obj_align).translate(tab)[::-1]}')
...
def aligner(full_df, target_sequence, align_method, filename, output_path, lliteral, rliteral, gop=3, gep=1, dir="3p", rev_complement="False"):
    ...
    aligned_data_reorg=reorganise_perc(aligned_data_trim)
    #Add primers to both ends of the seq and save
    #write_align(data_trim_nodupl, filename, target_sequence)

    add_primers_save2(aligned_data_reorg, filename, target_sequence, lliteral, dir, rev_complement)
...
def translate_nt_aa(result, corr_frame):
    ...
    for record in SeqIO.parse(result, "fasta"):
        if record.id=="0":
            refs_aa_frames["Frame_" + str(corr_frame)]=str(Seq(record.seq[corr_frame:]).translate())
            for alt_frame in out_of_frames:
                refs_aa_frames["Frame_" + str(alt_frame)]=str(Seq(record.seq[alt_frame:]).translate())
        else:
            aa_and_perc[">"+str(record.description) + "_transl.frame_" + str(corr_frame)]=str(Seq(record.seq[corr_frame:]).translate())
    ...
    def translate_nt(self):
        ...
        for record in SeqIO.parse(self.result, "fasta"):
            if "Ref" in record.description:
                ...
                ref_key="Frame_corr_" + str(self.corr_frame) +"|" +str(Seq(record.seq[self.corr_frame:]).translate())
            else:
        ...
        for alt_frame in alt_frames:
            ...
            for record in SeqIO.parse(self.result, "fasta"):
                if "Ref" in record.description:
                    #refs_aa_frames["Frame:" + str(alt_frame)]=str(Seq(record.seq[alt_frame:]).translate())
                    ref_key="Frame_" + str(alt_frame) +"|" +str(Seq(record.seq[self.corr_frame:]).translate())
                else:
...
def translate_NT(result, corr_frame, dir, out_csv, primer, aa_primer_frame, align_method="align_local3"):
    ...
    write_AA_file(df_aa_align, result, primer, dir,aa_primer_frame, corr_frame)
